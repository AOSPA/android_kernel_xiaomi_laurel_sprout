...
static int dsi_display_status_check_error_flag(struct dsi_display *display)
{
	...
	if (gpio_is_valid(display->esd_error_flag_gpio)) {
		...
		if (count >= GPIO_READ_MAX_TIMES ){
			pr_debug("%s:reading erro flag gpio is failing rc = %d\n",__func__,rc);
			return -EINVAL;
		}
	}
	return true;
...
static bool dsi_display_white_reg_read(struct dsi_panel *panel)
{
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
		  pr_debug("%d,return_buf[%d]",i,config->return_buf[i]);
		}
		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		pr_debug("enter SDE_MODE_DPMS_LP1\n");
	...
	case SDE_MODE_DPMS_LP2:
		pr_debug("enter SDE_MODE_DPMS_LP2\n");
		blank = MSM_DRM_BLANK_POWERDOWN;
		notifier_data.data = &blank;
		msm_drm_notifier_call_chain(MSM_DRM_EARLY_EVENT_BLANK,
					&notifier_data);
	...
	case SDE_MODE_DPMS_ON:
		...
		notifier_data.data = &blank;
		pr_debug("enter SDE_MODE_DPMS_NOLP\n");
		msm_drm_notifier_call_chain(MSM_DRM_EARLY_EVENT_BLANK,
					&notifier_data);
	...
	case SDE_MODE_DPMS_OFF:
		blank = MSM_DRM_BLANK_POWERDOWN;
		notifier_data.data = &blank;
		pr_debug("enter SDE_MODE_DPMS_OFF\n");
		msm_drm_notifier_call_chain(MSM_DRM_EARLY_EVENT_BLANK,
					&notifier_data);
		break;
	default:
