...
char *hardwareinfo_items[HARDWARE_MAX_ITEM] = {
	"LCD",	"TP",	   "MEMORY",   "CAM_FRONT",  "CAM_BACK",    "BT",
	"WIFI", "GSENSOR", "PLSENSOR", "GYROSENSOR", "MSENSOR",	    "SAR",
	"GPS",	"FM",	   "BATTERY",  "CAM_M_BACK", "CAM_M_FRONT", "BOARD_ID"
};
...
static long hardwareinfo_ioctl(struct file *file, unsigned int cmd,
			       unsigned long arg)
{
...
static ssize_t show_boardinfo(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	int i = 0;
